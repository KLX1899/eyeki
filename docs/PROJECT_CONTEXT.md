...
The repository provides scheduler, runtime reload, config-watch, and interval/config-persistence unit tests but no broader test suite, settings UI, snooze, reminder history, tray icon, network service, telemetry, localization catalog, automatic update mechanism, release automation, or verifiable published release.
...
The project is an early Linux prototype with separate configuration, config-watch, runtime, and scheduler modules plus preliminary local build/packaging files. It has no release tags or CI (automated builds and checks that run on repository changes). The scheduler, runtime, config-watch, and interval/config-persistence suites passed with a disposable GCC 15/Make toolchain on 2026-08-20. The full application also compiled without warnings against disposable GTK 3, libnotify, and libsystemd development packages, and its one-shot CLI plus notification-mode reload loop ran with an isolated home. The host still lacks installed compiler/Make commands and development `.pc` files, and reminder delivery was not exercised in a supported graphical session. The existing ignored binary is not a trustworthy release artifact. A one-shot CLI command performs one operation, such as changing or printing a setting, and then exits; it does not enter the long-running reminder loop. The source archive and Debian metadata are not release-ready.

Important correctness gaps include fragile session selection, silent notification and configuration-read failures, permissive mode parsing, concurrent settings-update races, and unsafe runtime switching into popup mode. “Fragile session selection” means EyeKi accepts the first session returned by logind instead of proving that it is the current user's relevant graphical session. On a machine with two signed-in users, an SSH session, or multiple seats, the first result may belong to somebody else, so EyeKi may reset or advance the timer using the wrong person's idle state. Elapsed scheduling and logind idle timestamps use a monotonic clock, interval input is range-checked before overflow-safe conversion, settings writes are private and atomic, and daemon runtime state reloads promptly from a config watch, but the implementation still needs full build and runtime evidence on supported Ubuntu sessions.
