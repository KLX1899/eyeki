...
- The daemon observes atomic configuration replacements with Linux inotify and resets monotonic active time before counting under the latest complete interval and mode.
- CLI operations are `--help`, `--version`, `--show-config`, `--set-interval <min>`, `--set-mode n|p`, and `--daemon`.
- systemd-logind supplies session metadata through `sd-login` and selected-session idle properties through system D-Bus.
- Session resolution accepts only an active local graphical user session owned by the process effective UID, preferring the process session and then logind's primary display before a unique fallback.
- A systemd user unit and preliminary Debian metadata exist locally.

The repository provides scheduler, runtime reload, config-watch, interval/config-persistence, and logind session-selection unit tests but no broader test suite, settings UI, snooze, reminder history, tray icon, network service, telemetry, localization catalog, automatic update mechanism, release automation, or verifiable published release.
...
The project is an early Linux prototype with separate activity selection, configuration, config-watch, runtime, and scheduler modules plus preliminary local build/packaging files. It has no release tags or CI (automated builds and checks that run on repository changes). The scheduler, runtime, config-watch, interval/config-persistence, and logind session-selection suites passed with a disposable GCC 15/Make toolchain on 2026-08-21. The full application also compiled with warnings treated as errors against disposable GTK 3, libnotify, and libsystemd development packages; its one-shot CLI and a twelve-second notification-mode idle-poll run completed as expected with an isolated home in the current graphical environment. The host still lacks installed compiler/Make commands and development `.pc` files, and reminder delivery plus multi-user/multi-seat behavior were not exercised in a supported Ubuntu test session. The existing ignored binary is not a trustworthy release artifact. A one-shot CLI command performs one operation, such as changing or printing a setting, and then exits; it does not enter the long-running reminder loop. The source archive and Debian metadata are not release-ready.

Important correctness gaps include silent notification and configuration-read failures, permissive mode parsing, concurrent settings-update races, and unsafe runtime switching into popup mode. Session selection no longer trusts global enumeration order: it verifies effective-UID ownership and active/local/graphical/user-class eligibility, with deterministic process-session and primary-display precedence plus an ambiguity-safe unique fallback. Elapsed scheduling and logind idle timestamps use a monotonic clock, interval input is range-checked before overflow-safe conversion, settings writes are private and atomic, and daemon runtime state reloads promptly from a config watch, but the implementation still needs full build and runtime evidence on supported Ubuntu sessions.
...
- **Active time:** elapsed time accumulated between polling samples while `activity_get_idle_seconds()` reports less than 60 seconds of idle time. This is an approximation, not verified keyboard/mouse or screen-on time.
...
- Keep the monotonic scheduler and current-session resolver covered while presentation and packaging work proceeds.
...
- Scheduler regression tests cover injected short intervals, threshold, idle reset, unknown-state reset, and clock-discontinuity behavior. Runtime/config-watch regressions cover interval and mode resets, invalid reload preservation, missing parent creation, repeated atomic replacements, and complete config loading. Interval/config tests cover production boundaries, malformed values, seconds conversion, XDG/legacy selection, private atomic round trips, and save failures. Session-selection tests cover UID ownership, process/primary-display precedence, local graphical eligibility, ambiguity, and empty/error results. They are not enforced by CI; broader desktop behavior still lacks repeatable automated evidence.
